...
const char * const BITCOIN_CONF_FILENAME = "xchangeum.conf";
const char * const BITCOIN_PID_FILENAME = "xchangeumd.pid";
...
#else
    const char* pszModule = "xchangeum";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "xChangeum";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/xChangeum";
#else
    // Unix
    return pathRet / ".xchangeum";
#endif
#endif
}
